...
- (MYCoreTextLabel *)coretextLabel
{
    if (!_coretextLabel) {
        ...
        model.textFont = [UIFont systemFontOfSize:14.f]; //设置文本大小
        model.textColor = [UIColor brownColor];       // 设置文本颜色
        model.lineSpacing = 2;    //设置行间距
        model.wordSpacing = 1.5;  //设置字间距
        model.keyWord = @"苍山";   //设置关键字
        model.keyWordColor = [UIColor redColor]; //设置关键字颜色
        model.keyWordBackColor = [UIColor brownColor]; //设置关键字背景高亮色 , 如果不需要可自行调整为透明 或者和背景色一样 , 默认是黄色的
        _coretextLabel.attribute = model;
        model.notShowNormalLink = YES; // notShowNormalLink为YES ,不显示所有的常规链接特性, 所以常规链接也都是直接以普通文本方式展示 , 并不可点击
